...
namespace CCGKit
{
    ...
    public class GameNetworkClient : NetworkBehaviour
    {
        ...
        protected virtual void RegisterNetworkHandlers()
        {
            ...
            client.RegisterHandler(NetworkProtocol.PlayerDrewCards, OnPlayerDrewCards);
            client.RegisterHandler(NetworkProtocol.OpponentDrewCards, OnOpponentDrewCards);
        }
        ...
        protected virtual void UnregisterNetworkHandlers()
        {
            if (client != null)
            {
                client.UnregisterHandler(NetworkProtocol.OpponentDrewCards);
                client.UnregisterHandler(NetworkProtocol.PlayerDrewCards);
        ...
        /// </summary>
        /// <param name="netMsg">Network message.</param>
        protected void OnPlayerDrewCards(NetworkMessage netMsg)
        {
            var msg = netMsg.ReadMessage<PlayerDrewCardsMessage>();
            Assert.IsTrue(msg != null);
            var player = localPlayers.Find(x => x.netId == msg.playerNetId);
            if (player != null)
            {
        ...
        /// </summary>
        /// <param name="netMsg">Network message.</param>
        protected void OnOpponentDrewCards(NetworkMessage netMsg)
        {
            var msg = netMsg.ReadMessage<OpponentDrewCardsMessage>();
            Assert.IsTrue(msg != null);
            var player = localPlayers.Find(x => x.netId == msg.playerNetId);
            if (player != null)
            {
